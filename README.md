...
A second strict depth audit compared the study corpus back against the official slides, labs and transcripts using a stronger standard than topic coverage. Gaps found in that audit were repaired in `knowledge/depth/`; see `docs/DEEP_COVERAGE_AUDIT.md`.
...
4. [`knowledge/depth/`](knowledge/depth/) — targeted supplements restoring details found missing by the strict second audit.
5. [`docs/DEEP_COVERAGE_AUDIT.md`](docs/DEEP_COVERAGE_AUDIT.md) — criteria, gaps, remediation and final deep-coverage verdict.
...
The study corpus covers sparse matrices; Python/NumPy/SciPy tooling; linear-algebra foundations and Gaussian elimination/`PA=LU`; basis changes and similarity; orthogonalization, QR and projectors; polynomial interpolation, convergence and least squares; stationary iterative solvers; Krylov subspaces, Arnoldi, FOM and GMRES; spectral clustering; power/inverse-power, shifts and deflation; PCA and k-means; Gershgorin localization; Lanczos; QR eigenvalue iteration; SVD, bidiagonalization, optimal low-rank approximation and pseudoinverse; and the PageRank project. The current assessment material specifies PageRank and PCA as the two mandatory projects.
